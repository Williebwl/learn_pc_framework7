...
if (typeof jQuery === "undefined") {
    ...
    b.prototype.show = function (h) {
        ...
        this.escape();
        this.resize();
        this.$element.one("click.dismiss.bs.modal", '[data-dismiss="modal"]', d.proxy(this.hide, this));
        ...
        this.backdrop(function () {
            ...
            f.enforceFocus();
            f.$element.children().eq(0).css({
                // 更改
                'margin-left': function () {
                    return (f.$element.width() - f.$element.children().eq(0).width() - 40) / 2 > 0 ? -f.$element.children().eq(0).width() / 2 : "0px"
                },
                'margin-top': function () {
                    return (f.$element.height() - f.$element.children().eq(0).height() - 40) / 2 > 0 ? -f.$element.children().eq(0).height() / 2 : "0px"
                },
                'top': function () {
                    return (f.$element.height() - f.$element.children().eq(0).height() - 40) / 2 > 0 ? "50%" : "0px"
                },
                'left': function () {
                    return (f.$element.width() - f.$element.children().eq(0).width() - 40) / 2 > 0 ? "50%" : "0px"
                }
    ...
    b.prototype.hideModal = function () {
        var e = this;
        this.$element.is('.await') || this.$element.hide();
        this.backdrop(function () {
    ...
    b.prototype.backdrop = function (i) {
        var h = this,
            $self = this.$element,
            iscom = $self.length && $self.get(0).nodeType === 8,
            isawait = $self.is('.await'),
            f = $self.hasClass("fade") || iscom ? "fade" : "",
            beforebsmodal = $self.data('before.bs.modal');

        if (this.isShown && this.options.backdrop && ($(".modal-backdrop.in").length === 0 || beforebsmodal && !beforebsmodal.auto || isawait)) {
            var e = d.support.transition && f;

            this.$backdrop = (beforebsmodal && beforebsmodal.$backdrop || d('<div class="modal-backdrop ' + f + '" style="' + (isawait ? 'background: none;z-index: 3600;' : '') + '" >' + (iscom ? '<div class="loading"/>' : '') + '</div>')).appendTo(this.$body);

            if (beforebsmodal && !beforebsmodal.auto) beforebsmodal.auto = !0

            iscom || isawait || this.$element.on("click.dismiss.bs.modal", d.proxy(function (j) {
    ...
    function c(e, f) {
        return this.each(function () {
            var i = d(this);
            var h = i.data("bs.modal") || this["bs.modal"];
            var g = d.extend({}, b.DEFAULTS, i.data(), typeof e == "object" && e);
            if (!h) {
                i.data("bs.modal", (this["bs.modal"] = h = new b(this, g)))
            }
            if (typeof e == "string") {
                h[e](f)
            } else {
